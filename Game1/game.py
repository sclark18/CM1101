...
def remove_punct(text):
    ...
    output  = ""
    for i in text:
        if i in string.ascii_letters or i in string.whitespace:
            output += i
    return output
...
def remove_spaces(text):
    ...
    if output[0] in string.whitespace or output[-1] in string.whitespace:
        if output[0] in string.whitespace:
            output = output[1:]
        if len(output) >1 and output[-1] in string.whitespace:
            output = output[0:-1]
...
def normalise_input(user_input):
    ...
    'help'
    """
    output = remove_spaces(remove_punct(user_input))
    output = output.lower()
    return output
